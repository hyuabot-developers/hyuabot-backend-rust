    ...
    pub fn new(route_stop_item: BusRouteStopItem) -> Self {
        ...
        Self {
            ...
            realtime: realtime_arrival_list.into_iter()
                ...
                .map(|realtime_item| BusViaRouteRealtimeItem::new(realtime_item, now)).collect(),
...
impl BusViaRouteRealtimeItem {
    pub fn new(realtime_item: BusRealtimeItem, now: NaiveDateTime) -> Self {
        Self {
            ...
            time: realtime_item.remaining_time as i64 - (now - realtime_item.last_updated_at).num_minutes(),
        }
    }
}
...
impl BusViaRouteTimetableList {
    pub fn new(route_stop_item: &BusRouteStopItem) -> Self {
        let timetable = BusTimetableItem::find_by_weekday_stop_and_route_id(
            &route_stop_item.route_id, &route_stop_item.start_stop_id, None, None).unwrap();
        Self {
            start_stop: BusViaRouteTimetableStartStop::new(BusStopItem::get_one_by_id(&route_stop_item.start_stop_id).unwrap()),
            departure_time: timetable.into_iter().map(|timetable_item| timetable_item.departure_time.to_string()).collect(),
        }
    }
}
