...
use crate::response::shuttle::route::{
    ShuttleLocationResponse, ShuttleRouteListResponse, ShuttleRouteResponse,
};
...
#[get("")]
pub async fn get_shuttle_route(
    route_query: web::Query<ShuttleRouteNameQuery>,
...
#[get("/{route_id}")]
pub async fn get_shuttle_route_by_id(
    ...
    let timetable =
        ShuttleTimeTableItem::get_timetable_by_route_name(route_id.borrow(), &period.period_type)?;
    Ok(HttpResponse::Ok().json(ShuttleRouteResponse::new(
        route, &weekday, &stop_list, &timetable,
    )))
}

#[get("/{route_id}/location")]
pub async fn get_shuttle_location_by_id(
    route_id: web::Path<String>,
) -> Result<HttpResponse, CustomError> {
    let route_id = route_id.into_inner();
    let period = ShuttlePeriodItem::get_current_period()?;
    ...
    let timetable =
        ShuttleTimeTableItem::get_timetable_by_route_name(route_id.borrow(), &period.period_type)?;
    Ok(HttpResponse::Ok().json(ShuttleLocationResponse::new(
        &weekday, &stop_list, &timetable,
    )))
}
