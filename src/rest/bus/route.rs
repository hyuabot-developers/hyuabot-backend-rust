...
use crate::response::bus::route::{
    BusRouteItemResponse, BusRouteListResponse, BusTimetableListResponse,
};
use actix_web::get;
use actix_web::web;
...
#[get("")]
pub async fn get_bus_route(
    route_query: web::Query<BusRouteNameQuery>,
...
pub async fn get_bus_route_timetable(
    route_stop_query: web::Path<(i32, i32)>,
) -> Result<HttpResponse, CustomError> {
    let query = route_stop_query.into_inner();
    let route_id = query.0;
    let start_stop_id = query.1;
    let timetable =
        BusTimetableItem::find_by_stop_and_route_id(route_id.borrow(), start_stop_id.borrow())?;
